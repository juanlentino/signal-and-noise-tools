...
- **`cadence-flags` → `sn_health`, domain `cadence`.** It reports STATE
  ...
  health check (#17) already consumes it exactly this way. The v10.32.0
  hardening (schedule floor + burst span-gate, this same release)
  makes the domain's signal trustworthy enough to expose.
