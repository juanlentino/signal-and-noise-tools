...
| E3 | Basic-auth to `/wp-abilities/` is blocked | memory `waf-blocks-basic-auth-on-abilities` | **ENFORCED at the edge** (measured 2026-09-12) / **witnessed** by Better Stack once the monitor exists | The rule exists — custom rule 4 of 5, Block, Active, on `http.request.uri` — **and fires**: on 2026-09-12 an `Authorization`-bearing GET from a non-origin host was answered **403 at the edge** (cf-ray, no `server-timing`) on both `/wp-json/wp-abilities/…` and `/?rest_route=/wp-abilities/…`; without the header, 401 from WordPress. The "0 events" counter was simply that nobody had ever sent one. What is **not** refused is the origin's own request — the plugin probe read `open` from the origin against the same rule, so no request the plugin sends can judge it (mechanism unproven: an IP Access Rule / WAF exception for `157.245.116.64` is the leading candidate). `inc/health-check-cf-security-headers.php` therefore no longer probes; it reads a Better Stack **witness monitor** (Authorization header from outside, expects 403; config verified, not just the name) and reports itself SKIPPED until one exists. | Delete the rule → the witness goes down; origin IP. |
...
Direct-to-origin (Cloudways IP, or a route-pattern miss) loses: CSP/HSTS/XCTO/XFO/Referrer-Policy (all five are Cloudflare-only by design, README "Security"); HTML `<meta name="tdm-reservation">`; `/.well-known/tdmrep.json`; `/license.xml`; the worker-authored `robots.txt`; Markdown content negotiation; web-bot-auth licence offers; the login-guard denylist; the beacon rate limiter; the abilities Basic-auth block; and every WebMCP/rights `_sn/` endpoint. Survives: REST TDM headers (origin-owned since v9.83.0), route removal, rendered-field stripping, the origin login slug, all `permission_callback`s. The `Content-Signal` string is pinned in both repos and equal today; there is no test that reads both.
...
- **The WAF Basic-auth block** — off-repo; `.github/security-scan-instructions.md` describes it. The rule is confirmed present **and measured in force from outside** (E3, 2026-09-12); it is gone on a direct-to-origin path, so the reviewer should continue to credit only the in-plugin guard.
...
1. **Security → WAF → Custom rules**: `Block Basic-auth on abilities API` exists — rule 4 of 5, Block, Active, matched on `http.request.uri` — and **fires for external callers on both URL spellings** (measured 2026-09-12; Security → Events now shows those two 403s). Still open: **why the origin's own requests are not refused** — check **Security → WAF → Tools (IP Access Rules)** and the custom rules' exceptions for `157.245.116.64`, and rule 1 (`Skip security for static assets`) for a Skip scope wider than its two path prefixes. Not urgent (the origin is the plugin, not an attacker), but it is the one fact in this row nobody has read. **Then create the witness**: Better Stack → HTTP monitor on `https://juanlentino.com/wp-json/wp-abilities/v1/abilities` (and a second on `https://juanlentino.com/?rest_route=/wp-abilities/v1/abilities`), request header `Authorization: Basic x`, monitor type "expected status code" = **403**; the health check reads it.
