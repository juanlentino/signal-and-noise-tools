...
| E3 | Basic-auth to `/wp-abilities/` is blocked | memory `waf-blocks-basic-auth-on-abilities` | **UNVERIFIABLE-HERE** | WAF custom rule "Block Basic-auth on abilities API" (shipped 2026-08-26). This rule is the **only** thing closing M4's seam for external callers. No probe verifies it exists; no repo holds it. Direct-to-origin skips it. | Delete the rule; origin IP. |
...
- **The WAF Basic-auth block** — memory only; `.github/security-scan-instructions.md` does not mention it, so the security reviewer cannot reason about M4.
...
- `tests/js/*.mjs` (8 files: `prov-verify-core.test.mjs`, `mobile-apps.mjs`, `analytics-composition.mjs`, …) and `tests/desktop-status-resilience.cjs` — never run in CI (`run.sh` globs `tests/*.php`; no `node` test step). Local-only by hand.
...
1. **Security → WAF → Custom rules**: `Block Basic-auth on abilities API` exists, action Block, expression `(http.request.uri contains "wp-abilities" and any(http.request.headers.names[*] == "authorization"))`. Confirm the expression covers the `?rest_route=` spelling (it should — `http.request.uri` includes the query string). This rule is the whole of gap 1 today.
