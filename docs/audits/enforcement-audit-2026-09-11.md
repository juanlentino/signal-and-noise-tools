...
| E3 | Basic-auth to `/wp-abilities/` is blocked | memory `waf-blocks-basic-auth-on-abilities` | **RULE CONFIRMED PRESENT / EFFECT UNMEASURED** | The rule exists: dashboard-verified 2026-09-12, custom rule 4 of 5, action Block, status **Active**, expression beginning `(http.request.uri contains "wp-abilities" and ...`. (A 2026-09-12 edit briefly marked this row REFUTED on the strength of the probe below plus a recollection; both were wrong and that edit is withdrawn.) **What is still unmeasured:** the rule shows **0 events**, as does custom rule 5, and `inc/health-check-cf-security-headers.php` is not refused by it. That probe calls `home_url()` **from the origin server**, and `cf-ray` on the response proves the response traversed Cloudflare, not that the custom-rule phase judged the request. Until a request is made from OUTSIDE the origin, "rule inert" and "prober exempt" are indistinguishable here. | Delete the rule; origin IP. |
...
- **The WAF Basic-auth block** — off-repo; `.github/security-scan-instructions.md` does not describe it, so the security reviewer cannot reason about M4. The rule is confirmed present (E3), but its **effect** is still unmeasured from an external vantage, so the reviewer should continue to credit only the in-plugin guard.
...
1. **Security → WAF → Custom rules**: `Block Basic-auth on abilities API` exists — rule 4 of 5, Block, Active, matched on `http.request.uri` (the correct choice: `http.request.uri.path` would leave the `?rest_route=` spelling unmatched, and the health probe tests only the `/wp-json/` spelling so it could not see that difference). **The open question is not existence but effect:** this rule and rule 5 (`SN mcp`) both show 0 events, and the in-plugin probe is not refused. Send an `Authorization`-bearing request from a host that is NOT the origin and check Security → Events; also confirm no IP Access Rule or WAF exception exempts the origin's own address, and that rule 1 (`Skip security for static assets`) does not skip more than its two path prefixes.
