...
| ~~3a~~ | ~~`cron_scheduled`/`cron_history` sections redesigned~~ **SUPERSEDED by `cron_health` (v13.52.0)** — the model the partition asked for; the detail rows stay local by verdict | v13.52.0 |
...
| ✅ 4 | SSRF Step 2 (v13.53.0) -> breached-credential rejection (v13.54.0 → v13.60.0) -> measurement weave 0–5 (v13.55.0 → v13.62.0) | shipped, one release each |
| ✅ 5 | Batch edit per D2 | v13.56.0 |
...
| ~~GSC drift section~~ **RESOLVED 2026-09-04 — flipped to `measured`** | The last unverified surface of the v13.11.0 work, now verified end to end. First row: `/notes` 6.3 → 11.5, drift 5.2, impressions 11 — clearing the 5.0-drift and 10-impression floors by a hair, so treat it as the instrument working rather than a finding to act on: an average position over ELEVEN impressions moves several points on a couple of deep-page appearances. Trustworthy because `gsc_history_stalled` (v13.89.1) passed in the same scan — the history feeding the comparison is current, not starved. **Re-read ~2026-09-11**: if `/notes` still drifts with more impressions behind it, that is a finding; if it reverts, it was sample noise |
