...
|---|------|------|------|--------------------|
| 1 | **MCP options-not-tools — Phase 0.2 + Phase 2** | plugin | M | Plan: [proposals/mcp-options-not-tools-2026-08-30-plan.md](proposals/mcp-options-not-tools-2026-08-30-plan.md). **0.2** adds a `dimensions` column so multi-entry calls stop recording NULL — today the honest-null rule means the more the consolidated tools are used AS DESIGNED (batched), the blinder per-section telemetry gets, and Phase 1 just added six sections. **Phase 2** is the `dismiss` change type on `sn-apply`, which closes the wave-1 dismissal gap the allowlist itself flags and **gates phase 10** of the consolidation program. Phase 0.1 and Phase 1 shipped in v13.44.0/v13.45.0 |
...
| ? | **SSRF guard — validate the whole rrset** | plugin | S | **Open defect, do first.** `sn_ssrf_host_blocked()` blocks every documented bypass class (decimal/hex/octal, `::ffff:`, NAT64, CGNAT — all verified 2026-08-31), but resolves with `gethostbyname()`, which returns ONE address from a multi-address rrset while cURL resolves again independently. Measured: `dns.google` → `8.8.4.4` from `gethostbyname()`, `8.8.4.4, 8.8.8.8` from `gethostbynamel()`. Step 1 is `gethostbynamel()` + block-if-any, negative-controlled against a stubbed `[public, internal]` rrset. Step 2 (connect-time pinning via `http_api_curl`/`CURLOPT_RESOLVE`) needs the hook's reachability VERIFIED, not assumed. Plan: [proposals/ssrf-guard-multi-address-2026-08-31.md](proposals/ssrf-guard-multi-address-2026-08-31.md) |
| ? | **Measurement weave — phases 0–3** | plugin | M | Shared path normalizer (three live spellings today, diverging on bare/empty inputs — a join across them drops rows silently), then GSC onto the read door as sections, into `snt_insights_collect_signals()`, and the TF-IDF↔GSC disagreement scan. All over data already synced daily and readable by no agent. Phase 4 (remote twins) and Phase 5 (enum drift check, independent) follow. Plan: [proposals/measurement-weave-2026-08-31.md](proposals/measurement-weave-2026-08-31.md) |
| ? | **Breached-credential rejection** | plugin | M | The one auth surface neither the edge guard nor `two-factor`/WebAuthn can see — `grep` finds no password policy anywhere in `inc/`. HIBP k-anonymity, live API only (the offline corpus is ~84.2 GB across 1,048,576 files, measured). Mode A set-time fail-CLOSED, Mode B login-time advisory fail-OPEN memoized against the stored hash. Plan: [proposals/breached-credential-check-2026-08-31.md](proposals/breached-credential-check-2026-08-31.md) |
