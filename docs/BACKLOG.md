...
| 2 | **MCP options-not-tools — Phase 3** | plugin | M | Four exclusions the options framing reverses (`merge_tags`, `clear_template_overrides`, `run_cron_event` allowlisted, `schedule_health_scan` fire-and-forget). Overturns written decisions, so it wants one confirming sentence first. `ai-orphan-apply` stays out — irreversibility is the one thing the gates cannot fix |
