...
| Watch | Reads |
|-------|-------|
| signed_agent window — **READ 2026-08-30, window CLOSED** | Full week: **311 / 13,238** verified (0.74% → 1.66% → 2.35%; marginal 2.57%, still rising). Survey item A2 (licence handshake) is **BUILD-RECOMMENDED, awaiting owner go-ahead** — not started. Two caveats carried: the denominator's composition changed when worker v1.22.0 began serving `/webmcp/bridge.js` on every HTML page 2026-08-28, and the KPI sums the aggregate (cap not firing — v13.35.0 measured totals 65,205 vs aggregate 64,825). **Next read is a SPLIT, not a repeat**: `identity.by_agent` / `by_surface` on `signal-noise/get-machine-readers-summary`, shipped in plugin v13.36.0 for exactly this. One agent or many decides whether the handshake is an ecosystem surface or a bilateral deal |
