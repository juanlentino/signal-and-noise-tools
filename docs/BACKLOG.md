...
| ✅ v13.55.0 + v13.57.0 | **Measurement weave — phases 0–3 SHIPPED; 4–5 open** | plugin | M | Shared path normalizer (three live spellings today, diverging on bare/empty inputs — a join across them drops rows silently), then GSC onto the read door as sections, into `snt_insights_collect_signals()`, and the TF-IDF↔GSC disagreement scan. All over data already synced daily and readable by no agent. Phase 4 (remote twins) and Phase 5 (enum drift check, independent) follow. Plan: [proposals/measurement-weave-2026-08-31.md](proposals/measurement-weave-2026-08-31.md) |
| ✅ v13.54.0 + v13.58.0 + v13.59.0 (Modes A+B); Phase 3 surface open | **Breached-credential rejection** | plugin | M | The one auth surface neither the edge guard nor `two-factor`/WebAuthn can see — `grep` finds no password policy anywhere in `inc/`. HIBP k-anonymity, live API only (the offline corpus is ~84.2 GB across 1,048,576 files, measured). Mode A set-time fail-CLOSED, Mode B login-time advisory fail-OPEN memoized against the stored hash. Plan: [proposals/breached-credential-check-2026-08-31.md](proposals/breached-credential-check-2026-08-31.md) |
