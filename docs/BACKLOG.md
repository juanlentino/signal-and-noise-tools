...
| ✅ v13.55.0 + v13.57.0 + v13.61.0 | **Measurement weave — phases 0–4 SHIPPED; 5 open** | plugin | M | Shared path normalizer (three live spellings today, diverging on bare/empty inputs — a join across them drops rows silently), then GSC onto the read door as sections, into `snt_insights_collect_signals()`, and the TF-IDF↔GSC disagreement scan. All over data already synced daily and readable by no agent. Phase 4 (remote twins) and Phase 5 (enum drift check, independent) follow. Plan: [proposals/measurement-weave-2026-08-31.md](proposals/measurement-weave-2026-08-31.md) |
