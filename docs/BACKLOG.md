...
| GSC drift section | **Measured 2026-09-03: `6.0 of 7 days across 7 snapshots`** — one day short, and the producer is healthy (7 snapshots over 6 days is daily, no gaps, retention 10 so nothing lost). Flips **2026-09-04** to the good zero or its first rows. Read `sn-status{search_drift}`: v13.88.2 made `accruing` carry `progress` {snapshots, span_days, needed_days}, so the state now says how far off it is. **If span is still 6.0 on the 4th the producer stalled** — that is the finding, not the wait. Last unverified surface of the v13.11.0 work |
