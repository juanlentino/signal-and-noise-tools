...
| Watch | Reads |
|-------|-------|
| signed_agent window — **CLOSED 2026-08-30, item A2 SHIPPED** | Full week: **311 / 13,238** verified (0.74% → 1.66% → 2.35%; marginal 2.57%, still rising) against a pre-ship forecast of ~0. Survey item A2 (licence handshake) **BUILT AND LIVE**: `sn-rights-signals` **v1.24.0** answers a verified agent with `TDM-Licence-*` terms keyed to the identity it proved (fail-open verified live; unsigned requests get the declaration and nothing keyed to them), theme **v12.12.0** makes it discoverable in `/llms.txt` + `/.well-known/agents.json`, plugin **v13.43.0** adds the `identity` fold. **The open question is no longer whether to build it — it is CONCENTRATION**: read `identity.by_agent` / `by_surface` from `signal-noise/get-machine-readers-summary` once the plugin is live on the site. One agent or many changes what the handshake is; and `by_surface` separates genuine adoption from traffic to `/webmcp/bridge.js`, which worker v1.22.0 began serving on every HTML page 2026-08-28, mid-measurement |
