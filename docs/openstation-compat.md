...
| `openstation_posts_window_query_args` | present: documented **Experimental** in the upstream hook reference (`docs/hooks-reference.md`, `apps/posts/parts/query.php`, 1.1.8). The Posts window trims every `/wp/v2/posts` fetch with a `_fields` allowlist; this filter is the documented way to "extend `_fields` to ship more columns". We APPEND `sn_provenance`, `sn_edge` and `meta._sn_evergreen` (the fields behind the Provenance, Edge and Evergreen columns, `openstation.postsWindow.columns`; core's nested `_fields` projection ships that one meta key alone; one list, `SNT_OS_POSTS_FIELDS`) and touch nothing else: `snt_os_posts_window_query_args()` in `inc/openstation-preferences.php`. Experimental status noted: if the arg shape changes, the column goes empty again, which `tests/openstation-preferences.php` cannot see; the live shell is the only witness (#1255). |
| `openstation.postsWindow.columns`, `openstation.postsWindow.toolbarTrailing`, `openstation.postsWindow.bulkActions`, `openstation.postsWindow.opened`, `openstation.postsWindow.dataLoaded` (JS, `wp.hooks`) | present: documented in the upstream hook reference under the Posts workspace's "JS extension points" (1.1.8, `apps/posts/parts/columns.ts`). Consumed by `assets/os-posts.js` only: `columns` for the Provenance, Edge and Evergreen cells; `toolbarTrailing` for the Attention pill (one `<os-button variant="ghost">`, resolved once per window, `ui.extras`); `opened` fetches the pill's count, `dataLoaded` refreshes it no more than once per 60 s. `wp.os.openWindow( 'signal-noise', { params: { section } } )` on click, the **Stable** window API; the app honours the param through `mount` / `reopen`. `bulkActions` (same table upstream) consumed by `assets/os-posts-reschedule.js` since 17.3.0: one entry, `snt-reschedule`, no `confirm` (the shell's `ctx.host.confirm` is yes/no only, so the date is asked in an `<os-modal>` holding a native `datetime-local` input, upgraded through `wp.os.loadComponents( [ 'os-modal' ] )`, **Stable**); the verdict is `wp.os.showToast( { message } )`, **Stable**; the write is `POST signal-noise/v1/openstation/reschedule` (`inc/batch-schedule-rest.php`, `snt_batch_schedule_apply()`, the classic bulk action's own write) behind `edit_others_posts`, sent through `wp.os.fetch( url, init )`, **Stable**, which stamps the shell's heartbeat-refreshed REST nonce at call time (the Attention pill's read moved onto it in the same cut; a nonce localized at page load goes stale in a long-open shell, the PWA case). Known ceiling: the filter is resolved once per window with no mode argument (`resolveBulkActions( defaults )` in `apps/posts/parts/columns.ts`), so the button also paints in the Pages window; `run()` reads the undocumented `desktop-mode-pages` root class (`apps/posts/parts/app.ts`, `rootClass`) and refuses with a toast, and the route skips anything that is not a post regardless. Not consumed: `statusSegments` (values go verbatim to core as `?status=`, so "needs attention" cannot be a tab). |
...
| `wp.os.fetch( input, init, opts )` | present: documented **Stable** in the upstream JavaScript reference ("every HTTP call from a plugin", `src/boot/tracked-fetch.ts`). Since #1601 the shared runner (`assets/snt-ability-run.js`) sends every ability call through it when `typeof window.wp.os.fetch === 'function'`, so the target window's title-bar status ring moves, the heartbeat-refreshed nonce is stamped at call time and a 401/403 reaches the shell's auth recovery; `wp.apiFetch` stays as the classic-page and editor-iframe path. The runner keeps apiFetch's contract on top of the native `Response` (parsed JSON resolves, the parsed `WP_Error` body rejects), so no caller changed. `opts.silent: true` on the desktop widgets' background refreshes and the provenance poll (`assets/provenance-admin.js`), which otherwise breathe the ring of whatever window is focused. `opts.source` is declared but not read at 1.1.10 (an upstream gap, not filed). |
| `wp.os.announceContentChange( type, action, ids, source )` | present: documented **Stable** in the upstream JavaScript reference (`src/broadcast.ts`), the typed producer for `os.<type>.changed`. Consumed since #1621 by `assets/os-posts-reschedule.js` (post/updated on a landed reschedule, plain, the script runs only in the shell) and `assets/health-suggest-actions.js` (attachment/deleted for the orphan apply, attachment/updated for alt text, post/updated for the rest, one seam in `callAbility`, behind a `typeof` guard because the classic leaves have no shell). Source tag `signal-noise`, not the app runtime's per-window tag, so the S&N app window still refreshes on an Apply from S&N Home. |
...
| *(not consumed pre-rename)* | `openstation_content_changes_record()` | `includes/content-changes.php`, documented **Stable** in the upstream hook reference ("Content-change realtime layer", the recorder for a plugin's own mutation paths). Called since #1621 from `inc/ai-alt-text-suggest.php` (attachment/updated after the alt-text meta write) and `inc/ai-orphan-suggest.php` (attachment/deleted after `wp_delete_attachment()`), both behind `function_exists`: the realtime layer has no post-meta publisher and records nothing on `delete_attachment`, so the Media Library window never learned of either write. |
...
**Rows seven and eight are called directly, not wrapped.**
...
The three preferences (`mio_tips`, `mio_help`, `mio_look`) live beside the two window switches in `inc/openstation-preferences.php` and OS Settings › Signal & Noise. Verified against the 1.1.9 sources: `docs/mio.md`, `docs/mio-window-assistant.md`, `src/native-url-remap.ts`, `src/window/dom.ts` (`el.id = \`wp-window-${ config.id }\``). Upstream gap filed as WordPress/openstation#820; fix proposed in #819 (open, unmerged as of 2026-09-20; the runtime's `open_url` skipped the remap registry). Until #819 ships in a release the site runs, `assets/os-settings-tab.js` claims kit doors to the plugin's own pages in the capture phase and the S&N app's `openLink` asks `tryNativeRemap` first (14.7.6, #1304).

## The view preference lives in the framework's store (#1605, OpenStation 1.1.10)

The Signal & Noise app's icons-or-list choice used to sit in `window.localStorage` (`sn-signal-noise-view`), read through at paint time: one browser's choice, so the phone PWA and the desk disagreed and cleared site data reset it. It now rides one more App Framework seam, Experimental at 1.1.10 like every seam above:

- `$os->stored( $key, $fallback )` / `$os->store( $key, $value )` (`includes/framework/app/class-os.php`; `docs/app-framework.md`, the `$os` table and the porting table's "User meta / options for the window's own preferences" row): durable per-user storage, the key namespaced by app id (`signal-noise:view`). The WordPress adapter (`includes/framework/app/wordpress/class-store.php`) keeps the whole `user` scope in one user-meta row, `openstation_app_store`, so the value follows the user across devices. The `mount` closure in `apps/signal-noise/signal-noise.os.php` seeds `state.view` from it, and a `view` server action clamps and stores the bound value.
- The wiring is the WP Explorer's own (`apps/my-wordpress/parts/actions.php` `mount()` / `view_action()`, `my-wordpress.os.ts` `renderViewSwitch()`): the client's `os-pick` handler runs the local `set-view` reducer for an instant switch, then `void ctx.dispatch( 'view' )` so PHP remembers it.

Why the server seed works where a client-side one did not: the mount dispatch runs the app's `mount` hook on the server (`includes/framework/app/class-runtime.php`) and answers with `'state' => $state->all()`, which `src/app-runtime/session.ts` `apply()` adopts. A value set in the server `mount` closure IS the hydrated state. v13.109.15 seeded from the client `mounted()` hook, a different moment, and lost to that same hydration. No migration of the old localStorage key: a viewer who had picked List starts on icons once and picks it again, after which it follows them. Pinned in `tests/openstation-app.php` (the seed, the action, the clamp) and `tests/openstation-app-client.php` (no `localStorage`, the pick-then-dispatch shape, the body painted from `state.view`).
