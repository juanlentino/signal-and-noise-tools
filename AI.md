...
- **Tag fit, weekly.** Tags are the one field a published note can still change (they are not prose; the signature covers none of it). One request per note that carries tags: a Score per attached tag (does the note touch what the tag names, so a reader browsing the tag would find it relevant, or was it attached for reach), the tag descriptions as the state. Content › Tags lists the misfits (the subject absent, under 0.5 of 2, read with confidence 0.7 or better; a lower confidence is a shrug, not a verdict) with a remove box; Jev proposes no tags, what a note carries is the owner's call (16.9.2 retired the proposals: the first pass read the corpus's broad facets onto every note). The advisory count rides the health scan; a wrong reading of a right tag is the description to fix. `jev-tags-now`, `jev-tags`.
