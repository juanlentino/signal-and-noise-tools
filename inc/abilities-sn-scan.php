...
const SNT_SN_SCAN_TYPES = array(
	...
	'emdash',
	// v13.27.0: the vocabulary source — undescribed in-use tags + zero-post
	// tags. TERM-level, not post-level: scope kinds other than "all" are
	// rejected in snt_sn_scan_resolve_scope() rather than silently ignored.
	// Closes the surface gap the tag_hygiene health check (v13.24.0) left:
	// its advisory count rendered, its NAMES rendered nowhere.
	'tag_hygiene',
	...
	'search_disagreement',
	// v14.7.0: markup matching a house convention's shape but not its form —
	// see inc/sn-scan-editorial-conventions.php; the detector is shared with
	// sn-validate's editorial_convention check.
...
add_action( 'wp_abilities_api_init', function() {
	...
	wp_register_ability( 'signal-noise/sn-scan', array(
		...
		'description'         => 'Consolidated read-only scan, absorbing block-migrations-scan, pattern-adoption-scan, duplicate-body-scan, near-duplicate-scan, and link-candidates, plus a sixth source (orphan_media, a pure-SQL detector previously reachable only via the AI-gated ai-orphan-suggest path). scan_type takes exactly ONE value per call — cost profiles differ by an order of magnitude (near_duplicate is O(n^2) pair comparison; block_migrations/pattern_adoption/duplicate_body/orphan_media are O(n) walks; link_candidates is O(n) artifact reads) and a caller should feel that rather than have it hidden behind a convenience flag. scan_type "anchor_violations" applies two BINARY link rules over the corpus (scheduled posts included): anchor_equals_sentence (an <a>\'s anchor text is identical to the full sentence containing it, terminal punctuation ignored) and heading_contains_link (any <a> inside an h1–h6; a heading wrapped entirely in a link reports under this rule only, never both). No thresholds and deliberately NO anchor/sentence length ratio — a ratio punishes short sentences, which are intentional in the house voice. No apply path yet: apply_hint is null, violations are fixed in the editor. candidate_id = sha256(scan_type + target identity + content fingerprint), hex, stable across runs on unchanged content — dismissal and diffing both depend on this. Ordering is always confidence DESC, candidate_id ASC; two runs against unchanged content return byte-identical candidate lists. For scan_type block_migrations, each candidate surfaces block_path on targets[] (and evidence): block fingerprints are position-bound (md5 over post_id|block_path|serialize_block), so same-post candidates MUST be applied in DESCENDING position order (later/deeper paths first) — applying top-to-bottom invalidates later candidates in the same post. This mirrors sn_apply change.payload.edits\' descending-splice algorithm (inc/sn-apply/batch-edits.php: locate every edit against the original content, then splice DESC by position so earlier offsets stay valid). freshness reports what actually happened: block_migrations/pattern_adoption/duplicate_body/near_duplicate/orphan_media always recompute live (no cache exists to read), so freshness is always "fresh" regardless of what was requested; link_candidates reads a prebuilt artifact and can never honor freshness:"fresh" without writing (this tool writes nothing, ever), so it always reports "cached". dismissed is always false: block_migrations/pattern_adoption\'s underlying detectors already exclude dismissed candidates before this tool ever sees them (the existing dismiss-candidate store), and the other four sources have no dismissal store at all yet (sn_dismiss is a later phase) — include_dismissed is accepted but has no effect on any scan_type today. apply_hint names the next-step apply tool with its required_args where a direct, fingerprint-safe apply path exists: signal-noise/sn-apply with change.type block_migration (for block_migrations) or pattern_adoption (for pattern_adoption). It is null for duplicate_body and near_duplicate (no apply path), for orphan_media (ai-orphan-apply sits on neither MCP door and sn-apply deliberately has no delete-attachment change type), and for link_candidates (its only apply path, ai-link-apply, validates a positional fingerprint that only an AI-mediated suggest call can produce — wiring it here would fail the fingerprint-parity guarantee, so it is deliberately null; see FINDINGS.md). Every response also carries detectors[]: the rules this scan_type actually ran, each {id, triggers_on}, where id is the identifier the detector\'s own code uses (pattern_type, migration_type, rule) so an entry correlates with the candidates it produced. This exists so an EMPTY candidates[] is readable. pattern_adoption returns zero across the whole corpus not because nothing is registered but because both its detectors key on core/quote and ordered core/list, and the corpus contains neither; detectors[] lets a caller see that rather than guess at it. scan_type "tag_hygiene" (v13.27.0) walks the TAG VOCABULARY: undescribed in-use tags (detector undescribed_tag) and zero-post tags (detector unused_tag; a zero-post undescribed tag reports ONCE, as unused — its fix is pruning, not describing), plus, since 16.9.2, the notes carrying more than 4 tags (detector over_ceiling, target_identity post:<id>, apply_hint null: trimmed in the editor). Vocabulary-level, so only scope.kind "all" applies (post_ids/modified_since are rejected, not ignored). Its apply_hint names the NEXT-STEP tool per detector: signal-noise/describe-tags for undescribed (an AI suggest whose output feeds signal-noise/apply-tag-description, the only-if-empty writer) and signal-noise/prune-unused-tags for unused — both on the rw door. This is the itemized surface for the tag_hygiene advisory health check, whose wp-admin rendering is a count with no names by IA design. scan_type "editorial_conventions" (v14.7.0) walks the corpus (scheduled included) for markup that matches a house convention\'s SHAPE but not its FORM: a trailing "<strong>Correction," paragraph without sn-correction, a list under H2 References without is-style-references, a first-block whole-<em> paragraph without sn-lead, an ordered list of <strong>Term.</strong> items outside the steps group, an inline SVG missing role/aria-labelledby/title/desc or painted in fixed hex. All deterministic; the registry it tests against is the theme\'s (sn-site-facts{editorial_conventions}), and without it the scan REFUSES (503) rather than reporting a clean corpus. Each candidate carries block_path, the position-bound block fingerprint, targets[0].content_hash and evidence.replacement; a class fix\'s apply_hint is sn-apply change.type block_replace with payload.block_path, whose dry run reports diff.ledger_impact "coalesces" because no prose changes. Form fixes (a wrap, a move, an SVG rewrite) carry apply_hint null.',
		'category'            => 'tools',
...
function snt_sn_scan_resolve_scope( $scope, $scan_type ) {
	...
	// v13.27.0: tag_hygiene walks the TERM vocabulary, not posts. A post-id
	// or date scope has no meaning against terms; accepting one and ignoring
	// it would report "scoped" results that were never scoped.
	if ( 'tag_hygiene' === $scan_type && 'all' !== $kind ) {
		return snt_sn_scan_scope_error( __( 'scan_type "tag_hygiene" walks the tag vocabulary; only scope.kind "all" applies.', 'signal-and-noise-tools' ) );
	}
...
function snt_ability_sn_scan_impl( $input ) {
	...
	return array(
		...
		// v13.6.0: what this scan LOOKED for, so an empty candidates[] is a
		// finding rather than a silence. See inc/sn-scan-detectors.php.
		'detectors'        => snt_sn_scan_detectors_for( $scan_type ),
		'candidates'       => $page,
