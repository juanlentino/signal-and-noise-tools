...
function sn_now_dossier_html( $sections, $updated ) {
	...
	// v10.37.5: plain left stack (site rule: a hero with no real side
	// content never gets a split — the side column would hold only the
	// meta line). The theme grid is :has(-side)-guarded, so dropping the
	// wrappers turns it off without a theme change.
...
function sn_uses_dossier_html( $groups ) {
	...
	// v10.37.5: plain left stack — same rule as /now (see there).
