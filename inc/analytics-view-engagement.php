...
function snt_analytics_render_view_engagement( $from, $to, $class ) {
	...
	snt_analytics_render_distribution( __( 'Connection RTT', 'signal-and-noise-tools' ), sn_analytics_distribution( 'rtt', $from, $to, $class ), __( 'No TCP round-trips in this range — HTTP/3 connections carry no RTT, so only HTTP/1–2 visitors are measured (needs worker v1.7.0 + traffic).', 'signal-and-noise-tools' ) );
	echo '</div>';
	$pctl_note  = ( strtotime( (string) $to ) - strtotime( (string) $from ) > 90 * DAY_IN_SECONDS )
		...
		: '';
	$pctl_empty = __( 'Percentiles need live Analytics Engine data for this window.', 'signal-and-noise-tools' );
	echo '<div class="sn-an-grid">';
