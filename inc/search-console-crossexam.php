...
function snt_gsc_crossexam( $days = 28 ) {
	...
	$ledger = snt_mr_summary_payload( (int) $days );
	if ( empty( $ledger['ok'] ) ) {
...
function snt_gsc_crossexam_reading( $x ) {
	...
	switch ( (string) $x['verdict'] ) {
		case 'gsc_without_crawler':
			return __( 'Google reports impressions, but the Worker logged NO search-engine fetches. Two instruments disagree about whether Google is fetching this site — most likely the edge is serving the crawler from cache without the collector seeing it, which makes the ledger blind rather than the site unvisited.', 'signal-and-noise-tools' );
