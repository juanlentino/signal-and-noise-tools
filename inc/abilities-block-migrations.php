...
add_action( 'wp_abilities_api_init', function() {
	...
	wp_register_ability( 'signal-noise/block-migrations-scan', array(
		...
		'description'         => 'Walks all published AND scheduled (status "future") post_type=post posts; identifies core/heading blocks at level 3+ with no preceding level-2 core/heading in the same post (heading-hierarchy-skip, WCAG 1.3.1 violation). RULE (rewritten 2026-08-14, supersedes the old "level 3 only" scope): the single-note template titles with H1, so the correct first-level body subhead is H2 — H4-without-H2 was never a legitimate house pattern, and the rule now flags ANY non-H2 first-level subhead (H3 and H4 alike). A canonical wp:heading h2 stores no level attr; the walker treats missing level as 2. One candidate is minted PER HEADING (each with its own block fingerprint — the apply contract replaces one block per call); heading-level changes are provenance-safe regardless, because the ledger signs normalized prose with markup stripped, so N applies to one Note coalesce to ZERO new signed versions. Scheduled posts are walked so a skip is fixable BEFORE it publishes as permanently dated + canonical. Returns candidates with fingerprints (current_level 3-6, target_level 2) for safe-apply concurrency control. Caches result per-user for 1 hour.',
		'category'            => 'tools',
