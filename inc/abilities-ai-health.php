	...
	wp_register_ability( 'signal-noise/ai-link-suggest', array(
		...
		'output_schema'       => array(
			...
			'properties' => array(
				...
				'reason'          => array( 'type' => 'string' ),
				'anchor'          => array( 'type' => 'string', 'description' => 'The mention exactly as it appears in the prose — the text Apply wraps. Empty when the mention sits inside an existing link (advice-only, v8.1.1).' ),
				'can_apply'       => array( 'type' => 'boolean', 'description' => 'True when the splice contract is usable; false when the mention already sits inside an <a> (v8.1.1 additive).' ),
				'position'        => array( 'type' => 'integer', 'description' => 'RAW post_content byte offset (resolved via the drift locator); -1 when advice-only.' ),
