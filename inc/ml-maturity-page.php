...
function sn_ml_maturity_layers() {
	return array(
		...
		'model'   => array( __( 'Model', 'signal-and-noise-tools' ), __( 'What is the model?', 'signal-and-noise-tools' ), __( 'Two signals, blended. Classical text statistics (term weighting, similarity and ranking of the TF-IDF and BM25 family) computed inside the site, and semantic vectors produced once per note by a hosted embedding model. The statistics are reproducible from the corpus alone; the vectors are not - they depend on a model this site neither hosts nor pins, so identical text is not guaranteed to give identical output across model revisions. Ranking arithmetic itself is exact and runs here', 'signal-and-noise-tools' ) ),
		'compute' => array( __( 'Compute', 'signal-and-noise-tools' ), __( 'Where does it run?', 'signal-and-noise-tools' ), __( 'Ranking runs inside the site, when a note publishes and on a nightly schedule. Vectors are produced by a Cloudflare-hosted model once per note when its text changes, then cached. Results are computed once and stored, so no reader request ever waits on any of it', 'signal-and-noise-tools' ) ),
...
function sn_ml_maturity_scope() {
	$scope = array(
		...
		'search'    => array( __( 'Ranked search', 'signal-and-noise-tools' ), 'live' ), // v10.20.1: ⌘K ranked palette (theme v11.2.0); v13.111.0: the /notes search box, BM25 over the same index.
