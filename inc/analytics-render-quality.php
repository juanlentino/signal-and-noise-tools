...
function snt_analytics_render_bot_breakdown( $bb ) {
	...
	if ( $nets_failed ) {
		...
		echo '<h4 class="sn-an-subh">' . esc_html__( 'Top bot networks', 'signal-and-noise-tools' ) . '</h4><table class="sn-an-table widefat striped"><thead><tr><th scope="col" class="manage-column column-primary">' . esc_html__( 'Network', 'signal-and-noise-tools' ) . '</th><th scope="col" class="num">' . esc_html__( 'Views', 'signal-and-noise-tools' ) . '</th></tr></thead><tbody>';
		foreach ( $nets as $n ) {
			echo '<tr><td class="column-primary">' . esc_html( (string) ( $n['value'] ?? '' ) ) . '</td>'
