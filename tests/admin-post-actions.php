...
require_once __DIR__ . '/../inc/north-star.php'; // sn_handle_north_star_save
...
// ─── sn_handle_tag_group_apply: the theme's headings, from Content › Tags (17.2.0) ─
echo "\nTest: sn_handle_tag_group_apply() files tags through the theme's meta\n";
pa_eq( 'tag_group_unavailable', sn_handle_tag_group_apply( array( 'group' => array( 5 => 'built' ) ) ), 'without the theme functions the handler refuses' );
if ( ! defined( 'SN_TAG_GROUP_META' ) ) { define( 'SN_TAG_GROUP_META', 'sn_tag_group' ); }
$GLOBALS['__tg_meta'] = array(); $GLOBALS['__tg_writes'] = array();
$GLOBALS['__tg_terms'] = array( 5 => (object) array( 'term_id' => 5, 'slug' => 'c2pa' ), 6 => (object) array( 'term_id' => 6, 'slug' => 'brand-new' ) );
function sn_notes_tag_group_ids() { return array( 'record', 'settles', 'built', 'work' ); }
function sn_notes_tag_group_of( $t ) { return $GLOBALS['__tg_meta'][ $t->term_id ] ?? ''; }
function sn_notes_tag_group_effective( $t ) { $m = sn_notes_tag_group_of( $t ); return '' !== $m ? $m : ( 'c2pa' === $t->slug ? 'record' : '' ); }
if ( ! function_exists( 'get_term' ) ) { function get_term( $id, $tax = '' ) { return $GLOBALS['__tg_terms'][ (int) $id ] ?? null; } }
if ( ! function_exists( 'update_term_meta' ) ) { function update_term_meta( $id, $k, $v ) { $GLOBALS['__tg_meta'][ (int) $id ] = $v; $GLOBALS['__tg_writes'][] = array( (int) $id, $v ); return true; } }
if ( ! function_exists( 'delete_term_meta' ) ) { function delete_term_meta( $id, $k ) { unset( $GLOBALS['__tg_meta'][ (int) $id ] ); $GLOBALS['__tg_writes'][] = array( (int) $id, null ); return true; } }
if ( ! function_exists( 'sanitize_key' ) ) { function sanitize_key( $s ) { return preg_replace( '/[^a-z0-9_\\-]/', '', strtolower( (string) $s ) ); } }
$GLOBALS['__test_caps'] = array();
pa_eq( 'tag_group_applied', sn_handle_tag_group_apply( array( 'group' => array( 5 => 'record', 6 => 'built', 99 => 'built' ) ) ), 'files the one that changes' );
pa_eq( array( array( 6, 'built' ) ), $GLOBALS['__tg_writes'], 'brand-new is written; c2pa already renders under record from the seed; 99 is no term' );
$GLOBALS['__tg_writes'] = array();
pa_eq( 'tag_group_nothing', sn_handle_tag_group_apply( array( 'group' => array( 6 => 'built', 5 => '' ) ) ), 'a value in force, and unfiling a seeded tag with no meta: nothing to write' );
pa_eq( array(), $GLOBALS['__tg_writes'], 'no write for either' );
pa_eq( 'tag_group_applied', sn_handle_tag_group_apply( array( 'group' => array( 6 => 'nonsense' ) ) ), 'an unknown id is unfiled: the filing is removed' );
pa_eq( array( array( 6, null ) ), $GLOBALS['__tg_writes'], 'the meta is deleted, never set to junk' );
$GLOBALS['__tg_writes'] = array(); $GLOBALS['__test_caps'] = array( 6 => false );
pa_eq( 'tag_group_nothing', sn_handle_tag_group_apply( array( 'group' => array( 6 => 'work' ) ) ), 'without edit_term on the tag nothing is written' );
$GLOBALS['__test_caps'] = array();
