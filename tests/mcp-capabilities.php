...
ok( is_array( $rw ) && count( $rw ) === 14, 'rw allowlist is exactly 14 slugs (16.7.0 + jev-tells-check + jev-tells-pass; 16.5.0 + jev-fit-now; 16.4.0 + jev-collision-check + jev-lane-map; 16.3.3 + jev-pass-now, the on-demand Jev pass; v13.25.0 WIDENED by 2, owner-directed: the tag-vocabulary pair describe-tags + apply-tag-description beside prune-unused-tags; v13.0.0 wave 2 had retired 4, wave 1 (v12.0.0) 26)' );
...
ok( count( $rw_plugin ) === 14, 'sanity: the pinned plugin rw list itself is 14 (16.7.0)' );
