...
// The enqueue is recorded by handle: the #1626 pins read it, not the localize
// call, because the duplicate rows came from the enqueue, and a build that
// still enqueues on the shell but skips the localize must stay red.
...
cpl_eq( null, cpl_run_enqueue(), 'shell request: nothing localized' );
cpl_true( ! in_array( 'snt-command-palette', $GLOBALS['__test_enqueued'], true ), 'shell request: snt-command-palette is not enqueued' );
$GLOBALS['__os_shell'] = false;
$GLOBALS['__os_chromeless'] = true;
cpl_eq( null, cpl_run_enqueue(), 'chromeless window request: nothing localized' );
cpl_true( ! in_array( 'snt-command-palette', $GLOBALS['__test_enqueued'], true ), 'chromeless window request: snt-command-palette is not enqueued' );
$GLOBALS['__os_chromeless'] = false;
cpl_true( is_array( cpl_run_enqueue() ), 'negative control: classic wp-admin still localizes the payload' );
cpl_true( in_array( 'snt-command-palette', $GLOBALS['__test_enqueued'], true ), 'negative control: classic wp-admin still enqueues snt-command-palette' );
