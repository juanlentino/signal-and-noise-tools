...
ok( strpos( $ov, 'sn-aw-nowtoday' ) !== false, 'overview: Right now renders the two-up Now/Today micro-stat' );
ok( strpos( $ov, '<span class="sn-aw-nt-v">7</span>' ) !== false && strpos( $ov, 'visitors now' ) !== false, 'overview: the "now" cell shows the realtime visitor count' );
...
$ov = cap( 'sn_aw_overview' );
ok( strpos( $ov, '<span class="sn-aw-nt-v">84</span>' ) !== false && strpos( $ov, 'views today' ) !== false, 'overview: today-so-far is the "today" cell of the Now/Today pair (last series bucket, zero new queries)' );
ok( strpos( $ov, 'views today so far' ) === false, 'overview: the old "N views today so far" sentence is gone' );
