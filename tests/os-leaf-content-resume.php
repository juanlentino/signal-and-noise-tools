...
ok( array( 'resume_pdf_generate', 'resume_save' ) === snt_leaf_actions( $kit ) && snt_leaf_actions( $classic ) === snt_leaf_actions( $kit ), 'the actions are resume_save and the separate resume_pdf_generate, as on the classic leaf' );
...
ok( 2 === substr_count( $kit, '<os-form' ) && false !== strpos( $kit, 'submit-label="Generate PDF"' ) && false !== strpos( $kit, 'os-action="post"' ) && false !== strpos( $kit, 'submit-label="Save resume"' ) && false === strpos( $kit, 'os-arg-pipeline' ), 'two os-forms (Save resume, Generate PDF) dispatching post through the admin-post pipeline, submit "Save resume"' );
...
ok( array( 'resume_pdf_generate', 'resume_save' ) === snt_leaf_actions( $kit ), 'the repeater posts nothing: add, remove and move are DOM operations, the one action is still resume_save' );
// Negative control: the action pin at the top can fail. A planted per-row
// server action would be a second action.
ok( array( 'resume_move', 'resume_pdf_generate', 'resume_save' ) === snt_leaf_actions( $kit . '<os-button os-action="post" os-arg-action="sn_resume_move">Up</os-button>' ), 'the action pin discriminates: a planted resume_move os-button reads as a second action' );
