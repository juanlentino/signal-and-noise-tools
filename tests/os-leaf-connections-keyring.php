...
ok( 16 === count( (array) kr_table_rows( $kit, 'Site secret and the workers' ) ) + count( (array) kr_table_rows( $kit, 'Cloudflare' ) ) + count( (array) kr_table_rows( $kit, 'Issued by others' ) ), 'sixteen rows across the three tables' );
ok( 15 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="cloudways_api_key"' ) && false !== strpos( $kit, '<os-option value="mr_read_token">Site secret and the workers · Machine Readers read token</os-option>' ), 'the select lists the fifteen rows a value can be set on, labelled by group; the wp-config-only Cloudways key is not offered' );
ok( false !== strpos( $kit, 'wrangler secret put SN_MR_READ_TOKEN' ) && 4 === substr_count( $kit, 'npx wrangler secret put' ) && false !== strpos( $classic, 'wrangler secret put SN_MR_READ_TOKEN' ) && false !== strpos( $kit, 'heading="Worker secrets"' ), 'the rail lists the four worker commands, on both leaves' );
...
ok( 'wp-config (SN_BETTERSTACK_API_TOKEN)' === $issued[0]['source'] && 14 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="betterstack_token"' ), 'a constant-locked row reads its constant and leaves the select' );
