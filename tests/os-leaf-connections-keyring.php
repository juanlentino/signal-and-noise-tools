...
ok( 21 === count( (array) kr_table_rows( $kit, 'Site secret and the workers' ) ) + count( (array) kr_table_rows( $kit, 'Cloudflare' ) ) + count( (array) kr_table_rows( $kit, 'Issued by others' ) ) && 'Workers AI token (embeddings)' === kr_table_rows( $kit, 'Cloudflare' )[4]['credential'], 'twenty-one rows across the three tables (16.3.0: the TypeSafe key under Issued; 16.2.0: the Bing Webmaster key under Issued; 15.11.0: the two Zenodo tokens; 15.3.1: the Workers AI token under Cloudflare)' );
ok( 20 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="cloudways_api_key"' ) && false !== strpos( $kit, '<os-option value="mr_read_token">Site secret and the workers · Machine Readers read token</os-option>' ), 'the select lists the twenty rows a value can be set on (16.3.0: + the TypeSafe key; 16.2.0: + the Bing key), labelled by group; the wp-config-only Cloudways key is not offered' );
...
ok( 'wp-config (SN_BETTERSTACK_API_TOKEN)' === $issued[0]['source'] && 19 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="betterstack_token"' ), 'a constant-locked row reads its constant and leaves the select' );
