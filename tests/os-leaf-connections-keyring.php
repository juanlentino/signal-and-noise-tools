...
ok( 19 === count( (array) kr_table_rows( $kit, 'Site secret and the workers' ) ) + count( (array) kr_table_rows( $kit, 'Cloudflare' ) ) + count( (array) kr_table_rows( $kit, 'Issued by others' ) ) && 'Workers AI token (embeddings)' === kr_table_rows( $kit, 'Cloudflare' )[4]['credential'], 'nineteen rows across the three tables (15.11.0: the two Zenodo tokens under Issued; 15.3.1: the Workers AI token under Cloudflare)' );
ok( 18 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="cloudways_api_key"' ) && false !== strpos( $kit, '<os-option value="mr_read_token">Site secret and the workers · Machine Readers read token</os-option>' ), 'the select lists the eighteen rows a value can be set on, labelled by group; the wp-config-only Cloudways key is not offered' );
...
ok( 'wp-config (SN_BETTERSTACK_API_TOKEN)' === $issued[0]['source'] && 17 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="betterstack_token"' ), 'a constant-locked row reads its constant and leaves the select' );
