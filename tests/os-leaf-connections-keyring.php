...
ok( 20 === count( (array) kr_table_rows( $kit, 'Site secret and the workers' ) ) + count( (array) kr_table_rows( $kit, 'Cloudflare' ) ) + count( (array) kr_table_rows( $kit, 'Issued by others' ) ) && 'Workers AI token (embeddings)' === kr_table_rows( $kit, 'Cloudflare' )[4]['credential'], 'twenty rows across the three tables (16.2.0: the Bing Webmaster key under Issued; 15.11.0: the two Zenodo tokens; 15.3.1: the Workers AI token under Cloudflare)' );
ok( 19 === substr_count( $kit, '<os-option value="' ) && false === strpos( $kit, '<os-option value="cloudways_api_key"' ) && false !== strpos( $kit, '<os-option value="mr_read_token">Site secret and the workers · Machine Readers read token</os-option>' ), 'the select lists the nineteen rows a value can be set on (16.2.0: + the Bing key), labelled by group; the wp-config-only Cloudways key is not offered' );
