...
// H: 16.7.2, the per-note map is gone; the corpus for the pairs group.
$GLOBALS['__c']['posts'] = array(); $GLOBALS['__c']['calls'] = array();
mkpost( 1, 'publish', 'One' ); mkpost( 2, 'publish', 'Two' ); mkpost( 3, 'publish', 'Three' );

// P: the lane map: the corpus once per chunk, one terse Noul per unordered pair, the rubric on the first question of a chunk only.
...
$m3 = sn_jev_lane_map();
ok( $m3['ok'] && 1 === $m3['requests'] && 3 === $m3['judged'] && 2 === $m3['pairs'] && 5000 === $m3['input_tokens'] && 1 === count( $GLOBALS['__c']['calls'] ), 'P5 ONE request for three notes, two pairs at or above the line' );
...
$l3 = sn_jev_lanes();
ok( 1 === $l3['requests'] && 0.9 === $l3['pairs'][0]['noul'] && 1 === $l3['pairs'][0]['a'] && 2 === $l3['pairs'][0]['b'] && 'Two' === $l3['pairs'][0]['b_title'] && 0.5 === $l3['pairs'][1]['noul'], 'P7 pairs sorted desc with titles; 0.5 exactly counts; requests stored' );
...
ok( ! array_key_exists( 'mode', snt_ability_jev_lane_map( array( 'mode' => 'notes' ) ) ), 'P9 16.7.2: no mode; the ability ignores one' );
...
ok( $l['ok'] && $l['mapped'] && 0 === count( $l['pairs'] ) && 1 === $l['failed'], 'I4 jev-lanes hands the stored map out (the failed one from P8: zero pairs, failed=1)' );
