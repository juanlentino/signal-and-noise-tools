<?php
/**
 * One-time removal of orphaned options (inc/migrate-orphan-options.php).
 *
 * v5.0.0: the orphaned `sn_login_rewrites_flushed` option.
 * v9.68.0: the retired `sn_analytics_landing_preview` flag (the v9.67.0
 * preview toggle whose machinery the graduated Overview removed).
 *
...
// v9.68.0: the retired Overview landing-preview flag. v9.67.0 shipped the
// flag-gated preview tab behind `sn_analytics_landing_preview`; v9.68.0
// graduated the tab to the permanent default and removed the whole flag
// machinery — a site that had the preview ON keeps a dormant row nothing
// reads anymore (the save handler only deleted it when toggled OFF).
// Same sentinel-gated pattern, pinned the same ways.
$GLOBALS['__test_options']['sn_analytics_landing_preview'] = 1;
unset( $GLOBALS['__test_options']['sn_orphan_options_removed_v968'] );
sn_migrate_remove_landing_preview_orphan();
oo_eq( false, array_key_exists( 'sn_analytics_landing_preview', $GLOBALS['__test_options'] ), 'v968: landing-preview orphan deleted' );
oo_eq( 1, $GLOBALS['__test_options']['sn_orphan_options_removed_v968'] ?? null, 'v968: sentinel set after migration' );

// Idempotent: re-seed; sentinel present → no second delete.
$GLOBALS['__test_options']['sn_analytics_landing_preview'] = 1;
sn_migrate_remove_landing_preview_orphan();
oo_eq( true, array_key_exists( 'sn_analytics_landing_preview', $GLOBALS['__test_options'] ), 'v968: idempotent — sentinel gates re-run (no second delete)' );
