...
const out = process.env.ANALYTICS_ARTIFACTS || '/tmp/snt-analytics-responsive';
...
const css = ['variables','desktop','window-chrome','app-runtime','mobile'].map(f=>read(path.join(shell,`assets/css/${f}.css`))).join('\n')+'\n'+['assets/analytics/analytics-admin.css','assets/os-app.css','apps/sn-analytics/sn-analytics.css'].map(f=>read(path.join(root,f))).join('\n');
...
for (const [width,height,appWidth,appHeight,mobile] of cases) {
 for (const warning of [false,true]) {
  ...
  await page.setContent(`<!doctype html><html data-os-mode="${mobile?'mobile':'desktop'}" data-os-display="${mobile?'standalone':'browser'}"><meta name="viewport" content="width=device-width,initial-scale=1"><body class="os-active os-admin-bar-hidden"><style>html,body{margin:0;height:100%;overflow:hidden}*{box-sizing:border-box}${css}</style><div id="os-shell" class="os-shell" data-os-mobile-state="app"><header class="os-mobile-top">S&N Analytics</header><div class="os-shell__body"><div id="os-area" class="os-area"><div class="os-window os-window--native os-window--focused ${mobile?'os-window--maximized':''}" style="width:${appWidth}px;height:${appHeight}px;left:0;top:0"><div class="os-window__titlebar">S&N Analytics</div><div class="os-window__tabs"><div class="os-window__tab os-window__tab--active">Overview</div><div class="os-window__tab">Content</div><div class="os-window__tab">Campaigns</div></div><div class="os-window__body os-window__body--native"><os-stack gap="12" padding="0"><os-tabpanel for="main"><div class="os-app" data-os-app="sn-analytics">${initial.html}</div></os-tabpanel><os-tabpanel for="other" hidden><div class="os-app" data-os-app="sn-analytics"></div></os-tabpanel></os-stack></div></div></div></div><nav class="os-mobile-tabs">Home · Signal & Noise · S&N Home · Open apps</nav></div></body></html>`);
