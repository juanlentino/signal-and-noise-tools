...
### New
- **A north star: weekly engaged readers.** One number for what the site is for: human visitors, counted per day, who read at least one core page (notes, provenance, resume, about by default) past 50% scroll OR 30 seconds. Listings (the notes index, its pages, tags, feed) never count. It heads S&N Home with the change against last week and the three-week average, and under it three layers adapted from the vanity-versus-business-metrics frame to a site that sells nothing: **intent** (deep readers of 2+ core pages, `download`/`outbound` actions, resume and contact visits), **return** (readers per note published over four weeks; DOI downloads, the difference between daily snapshots of Zenodo's public stats for every record by the author ORCID, labelled "all time" until a week of snapshots exists; inquiries, the forms plugin's read and unread entries, never spam, partial or trash), and **inputs** (notes published, RSS readers, search clicks). The sections and both floors are settings (Measurement › Analytics › North star). Read ability `signal-noise/north-star` (read door 49 to 50), its rows on the SN Site Views widget beside the views they put in context (no separate widget; its budgeted height 510 to 620), a daily `snt_nsm_zenodo_daily` cron, a one-hour cache so polls never re-query the Analytics Engine. The visitor hash rotates daily, so a reader on two days counts twice; the card says so. `tests/north-star.php` pins the tally, with negative controls on the OR and on the intent count.
