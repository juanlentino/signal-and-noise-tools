...
> **Known residual (accepted, consistent with the codebase):** DNS-rebinding and decimal/octal encodings of the metadata IP rely on WP core's resolve-time checks; `inc/webhooks.php` + `inc/uptime-heartbeat.php` share WP core's `169.254` omission (lower impact — webhooks are HMAC-signed). Extending the explicit link-local block to those two is a cheap follow-up.
