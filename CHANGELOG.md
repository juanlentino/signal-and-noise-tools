...
- **`tests/analytics-realtime.php` (22 assertions)** covering the realtime SQL builder, the int-vs-null accessor (a warmed zero is a real `0`, distinct from "not warmed"), the refresh's no-poison-on-failure guarantee across null / non-array / empty / missing-key shapes plus int clamp/coerce, and the warmer's scheduling decisions.
