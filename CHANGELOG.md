...
> **Why PATCH:** refactor/fix — no new capability, no visual or copy change; the default locale renders byte-for-byte identically to v9.42.0.

### Fixed
- i18n: why/empty diagnostics, column headers, and inline labels across [inc/analytics-render-tables.php](inc/analytics-render-tables.php), [inc/analytics-render-events.php](inc/analytics-render-events.php), [inc/analytics-render-quality.php](inc/analytics-render-quality.php), [inc/analytics-render-anomalies.php](inc/analytics-render-anomalies.php), [inc/analytics-render-geography.php](inc/analytics-render-geography.php), [inc/analytics-render-distribution.php](inc/analytics-render-distribution.php) and the five view files (content / geography / technology / engagement / quality); the dashboard-wide zero-data notice in [inc/analytics-admin.php](inc/analytics-admin.php); the two Dashboard-home widget empties in [inc/analytics-widget.php](inc/analytics-widget.php).
- i18n: the drill-down title becomes a proper sprintf msgid (`Top pages · %1$s = %2$s`, translators comment attached) and its 11-entry dimension-label map is translatable ([inc/analytics-render-drilldown.php](inc/analytics-render-drilldown.php)); the `&larr;` entity stays outside the msgid.
- i18n: the two settings locked-constant notes share one `Locked by the %s constant.` msgid via sprintf with translators comments — the `<code>` markup stays live, outside the msgid ([inc/analytics-render-settings.php](inc/analytics-render-settings.php)).
- Comments: the five stale `D5 §5` primitive-adoption citations in [inc/login-defense-analytics.php](inc/login-defense-analytics.php) now cite `D5 §2` (§5 is the D5 perf memo, §2 is the adoption section; comment-only, no code line moved).

### Added
- [tests/analytics-i18n.php](tests/analytics-i18n.php): a 63-assertion dual-oracle suite — recording `__`-family stubs drive the REAL renderers (a string counts as translatable only when it arrives through the recorder with the plugin domain, while the rendered English stays byte-identical), plus a source contract pinning the wrapped form at every heavy-fixture call site and the § citation fix. Three existing harnesses gained guarded i18n stubs: [tests/analytics-dimtable-render.php](tests/analytics-dimtable-render.php), [tests/analytics-drilldown-render.php](tests/analytics-drilldown-render.php), [tests/analytics-recommendations.php](tests/analytics-recommendations.php).
...
> **Why PATCH:** a correctness fix to an existing aggregation, no new user-visible capability. No public function, REST route, ability, or settings-schema change; no WP-floor raise. Fails safe: when the site uses a manual UTC offset (not a named zone) or AE rejects the timezone syntax, every query degrades to the exact pre-v9.26.4 behaviour (a same-run UTC fallback in both the rollup and the realtime refresh), so a zone-syntax problem can never blank or corrupt data. The daily-table DB version bumps 3→4, but the **schema is unchanged** — the migration only schedules a one-time re-roll of the trailing window to overwrite UTC-keyed buckets with local-day ones (idempotent by `(day, path, class)`, history preserved), requiring no user action.

### Fixed
- **Beacon daily rollup buckets by the site-local day** ([inc/analytics-rollup.php](inc/analytics-rollup.php)): `sn_analytics_rollup_sql()` gains a `$tz` arg — with a named zone it selects `formatDateTime(timestamp, '%Y-%m-%d', '<tz>')` and floors the window to `toStartOfInterval(now(), INTERVAL '1' DAY, '<tz>')`, so `wp_sn_analytics_daily` keys on the site's calendar day. `sn_analytics_run_rollup()` resolves the zone and, on a zoned-query failure (e.g. HTTP 422 on an older AE), re-runs the UTC form within the same pass so the table never goes stale.
- **Live "views today" uses the exact local-day start** ([inc/analytics-realtime.php](inc/analytics-realtime.php)): `sn_analytics_views_today_sql()` gains a `$tz` arg — a named zone uses `toStartOfInterval(now(), INTERVAL '1' DAY, '<tz>')` (no PHP-computed elapsed, no `now()`/`time()` skew); the refresh retries the elapsed-seconds form if the zoned query fails.
- **Shared zone resolver** ([inc/analytics-api.php](inc/analytics-api.php)): new `sn_analytics_site_tz_name()` returns the site's IANA identifier validated against `timezone_identifiers_list()` (so a manual offset like `+05:30`, or any non-identifier, yields `''` → the UTC/elapsed path) — validation that also guarantees the string is injection-safe before it reaches SQL.
- **One-time re-roll migration** ([inc/analytics-rollup.php](inc/analytics-rollup.php)): DB version 3→4 schedules a single `SN_ANALYTICS_ROLLUP_HOOK` event so the trailing window re-aggregates into local-day buckets promptly; no drop, no purge, schema unchanged.

### Tests
- [tests/analytics-rollup.php](tests/analytics-rollup.php) — zoned vs UTC day-bucket + window shapes, an injectable zone rejected to the UTC path, and the v3→v4 migration (no drop/purge, schedules exactly one re-roll, stamps 4).
- [tests/analytics-realtime.php](tests/analytics-realtime.php) — zoned `views today` lower bound via `toStartOfInterval`, injectable zone rejected to the elapsed window.
...
- **Session-quality rollup filters pageview-less groups, matching the live view** ([inc/analytics-session-rollup.php](inc/analytics-session-rollup.php)): `sn_session_rollup_run()` now runs `sn_pageview_visits()` before `sn_session_metrics()` — exactly as `snt_analytics_render_view_sessions()` does — so `wp_sn_session_daily` (bounce_pct / ppv / median_dur) no longer counts RSS/server pageview-less groups and agrees with the interactive Visits view for the same window.
- **Edge-dims upsert chunks its INSERT** ([inc/edge-rollup.php](inc/edge-rollup.php)): `sn_edge_dims_upsert()` now batches at 100 rows/statement (500 placeholders) like every sibling upsert, instead of one INSERT for the full unbounded backfill set that could exceed MySQL's per-statement placeholder limit and silently write zero dims.
- **"Today so far" pulse cell only shows when the last bucket is today** ([inc/analytics-header-region.php](inc/analytics-header-region.php)): `snt_analytics_render_pulse_strip()` suppresses the cell unless `end($series)`'s day equals today (UTC), so a not-yet-rolled current day no longer mislabels yesterday's full-day count as "Today so far."

### Tests
- [tests/analytics-realtime.php](tests/analytics-realtime.php) — durable last-good group (8 assertions incl. a direct reproduction of the 55→40 regression: lapsed transient returns the same-day value, a different-day value is treated as stale, a failed refresh neither clobbers nor drops to null); option stubs added to the harness. 50 assertions.
- [tests/analytics-session-rollup.php](tests/analytics-session-rollup.php) — `run()` wiring test proving metrics see only the pageview-filtered visits (phantom pageview-less group dropped).
- [tests/edge-rollup.php](tests/edge-rollup.php) — 250 dims rows chunk into 3 INSERTs, none carrying more than 100 rows, with the total-written accumulated across chunks.
...
> **Why MINOR:** new user-visible capabilities (a monthly-budget setting + readout, and cheaper default routing for short features), additive. No public function/ability/REST route removed or renamed. The picker drops Sonnet 4.6, but that is not a settings-schema break: a stored `claude-sonnet-4-6` still validates through the existing allowlist fallback to the default (Sonnet 5), so no user action or migration is required. The cost model only gets cheaper; nothing changes until an owner sets a budget. No WP-floor raise.

### Added
- **Monthly AI spend cap** ([inc/ai-bootstrap.php](inc/ai-bootstrap.php)): a durable month-to-date rollup (`SN_AI_SPEND_ROLLUP_OPT`, keyed `YYYY-MM`, pruned to `SN_AI_SPEND_MONTHS`) that `snt_ai_record_usage()` folds each call's cost into — immune to the FIFO usage log's 200-entry eviction, so month-to-date stays exact under heavy use. `snt_ai_generate_with_constraints()` gains a gate: when `theme.ai_monthly_budget > 0` and this month's spend has reached it, it returns `WP_Error('snt_ai_over_budget', 402)` before any model call, so every AI feature is capped uniformly. New helpers `snt_ai_spend_this_month()`, `snt_ai_add_month_spend()`, `snt_ai_spend_month_key()`.
- **Budget setting + readout** ([inc/settings.php](inc/settings.php), [inc/admin-forms/front-end.php](inc/admin-forms/front-end.php), [inc/admin-post-actions.php](inc/admin-post-actions.php), [inc/insights-admin.php](inc/insights-admin.php)): `theme.ai_monthly_budget` (USD, default 0 = off) renders on the Front-End settings form with a live "this month so far" figure and saves with a `>= 0` cents clamp; the AI usage & spend section gains a "This month: $X of $Y (Z%)" line with an over-budget state.
- **Economy-tier model routing** ([inc/ai-bootstrap.php](inc/ai-bootstrap.php)): `snt_ai_register_economy_model_route()` — a `snt_ai_model_preference` filter at priority 20 (a hard floor over the owner's model dropdown at priority 10) that routes the `snt_ai_economy_features()` set (meta_desc, excerpt, og_title, drift_phrase, tag_suggest) to Haiku 4.5. Both the feature list and the model are filterable (`snt_ai_economy_features`, `snt_ai_economy_model(model, feature, inherited)`); the escape hatch can opt a feature back onto the owner's pick. Mirrors the existing alt-text route.

### Changed
- **Per-feature AI cost attribution** ([inc/ai-meta-description.php](inc/ai-meta-description.php), [inc/ai-excerpt.php](inc/ai-excerpt.php), [inc/ai-og-card-title.php](inc/ai-og-card-title.php), [inc/ai-drift-phrase-suggest.php](inc/ai-drift-phrase-suggest.php), [inc/ai-link-suggest.php](inc/ai-link-suggest.php), [inc/ai-orphan-suggest.php](inc/ai-orphan-suggest.php), [inc/ai-pair-suggest.php](inc/ai-pair-suggest.php), [inc/health-checks.php](inc/health-checks.php), [inc/insights.php](inc/insights.php), [inc/release-notes-draft.php](inc/release-notes-draft.php)): the 11 call sites that passed no `$feature` label (defaulting to `generic`) now pass a distinct one — so the usage log's per-feature breakdown separates the 2048-token Insights scan from a 60-token drift phrase instead of bucketing them together. Purely additive; behaviour unchanged.
- **AI fallback model repinned to Sonnet 5** ([inc/ai-bootstrap.php](inc/ai-bootstrap.php), [inc/admin-post-actions.php](inc/admin-post-actions.php)): `SN_AI_FALLBACK_MODEL` moves from `claude-sonnet-4-6` to `claude-sonnet-5` (same $3/$15 list price, strictly better, now universally resolvable). Sonnet 4.6 is dropped from the `sn_theme_ai_models()` picker (now Sonnet 5 / Opus 4.8 / Haiku 4.5); a stored `claude-sonnet-4-6` degrades to the default via the existing allowlist validation. The pricing map keeps 4.6 (and other retired ids) for historical cost attribution.
...
> **Why MINOR:** `/now` becomes a CMS Page authored from the existing text box. No settings-schema change, no API removed. Retry-safe (waits for the text box to have content) and the one-time migration never clobbers an existing, owner-edited Page.

### Added
- `sn_now_sync_page()`: regenerates the `now` Page from the current Content → Now Page box; wired into `sn_now_page_save()` so the plain-text box stays the authoring surface while the Page is the rendered artifact + Excerpt/SEO/URL surface. No-op when the box has no usable sections (never blanks the page) ([inc/content-migrations.php](inc/content-migrations.php), [inc/now-page.php](inc/now-page.php)).
- `sn_now_build_body()` + `sn_now_upsert_page()`: shared body build (hero + sections) and create-or-update (`post_status=publish`, `page_template=page-now`, seeded Excerpt; a full `post_content` replace on regenerate). Used by both the on-save sync and the one-time migration ([inc/content-migrations.php](inc/content-migrations.php)).
- `sn_migrate_now_page()` (`admin_init`, idempotent): the one-time initial carry-over. Guard order: flag, then existing-non-empty (bail and flag, never clobber), then content not ready (retry, no flag), then build + upsert + flag ([inc/content-migrations.php](inc/content-migrations.php), [inc/content-surfaces.php](inc/content-surfaces.php)).
- `sn_now_sections_to_blocks()`: pure converter turning the parsed `[{label, items[]}]` sections into a constrained group of `core/heading` + `core/list` blocks. Every label and item is `esc_html`'d at the block-markup boundary ([inc/content-migrations.php](inc/content-migrations.php)).
...
### Notes
- **Editing model:** `/now` is edited from **Content → Now Page** (the plain-text box), which regenerates the Page body on save. Because the box is the source of truth, editing the `/now` Page directly in the block editor is not the workflow (a later text-box save would regenerate the body). The hero (eyebrow/headline/dek) is fixed in the seed. This differs from the prose CMS pages (About, etc.), which are authored in the block editor.
- Because the editor now writes the Page, `inc/now-page.php` does **not** retire in Phase 3 (revising the earlier plan). Its now-vestigial `sn_now_sections` / `sn_now_updated` theme-filter callbacks are left in place (harmless once the theme filter is gone) and prune in a later cleanup.
- Tests: `tests/now-page-migration.php` (38 assertions: converter validity and escaping, build/upsert/sync, Page-creation args, excerpt, idempotency, retry-safety, the post-date backdate). Full sweep 251 suites, 6907 assertions.
...
> **Why MINOR:** additive admin capabilities (status panel, re-anchor button, commits table) plus a self-heal fix — no public API removed or renamed, no settings-schema change. The re-anchor re-sends the **persisted** genesis root verbatim; it never recomputes a root or leaf, so the attestation is unchanged. Self-heal marks a migration done only after a successful dispatch, so a missing-config deploy retries instead of silently sticking. The re-anchor button is nonce- + `manage_options`-gated, and the status panel surfaces only booleans for the secret (the HMAC value never reaches output).

### New
- **Provenance system-status card** ([inc/provenance-admin.php](inc/provenance-admin.php)): `sn_prov_admin_system_status()` — inferred Worker reachability (any commit/genesis that reached pending/confirmed ⇒ reachable + last-contact timestamp), config-presence booleans (worker URL / HMAC / public key — presence only, never the value), pending/confirmed/unanchored counts, and ledger + public-key links.
- **Genesis Re-anchor button** ([inc/provenance-admin.php](inc/provenance-admin.php)): a nonce- + `manage_options`-gated `admin_post_sn_prov_reanchor` action that re-sends the persisted genesis root from the UI.
- **Live commits table** ([assets/provenance-admin.js](assets/provenance-admin.js)): the 30 s poll now renders a `.sn-status-table` (UID · version · status pill coloured pending=amber / unanchored=red · per-Note ledger link), built with safe DOM methods (no `innerHTML`).
- **Card-based redesign** ([assets/provenance-admin.css](assets/provenance-admin.css)) reusing the plugin's existing `.sn-card` / `.sn-pill` / `.sn-status-table` design system rather than a bespoke look.

### Fixed
- **Self-healing genesis anchor** ([inc/provenance-genesis.php](inc/provenance-genesis.php)): `sn_prov_dispatch_manifest()` and `sn_prov_genesis_anchor()` now report whether the root was actually dispatched (status is `pending` only when sent, else `unsent`), and `sn_prov_genesis_migrate()` marks itself done only after a successful anchor — so a deploy that precedes the `SN_PROV_*` config no longer strands the genesis root. A new `sn_prov_genesis_reanchor()` reconstructs the manifest from the persisted root + each Note's v0 commit (never recomputing the root) and re-sends it, wired to run once on `admin_init` (gated by `sn_prov_genesis_reanchored_v1`) and from the admin button. A successful initial anchor closes the re-anchor gate in the same request to avoid a redundant double-dispatch on fresh installs.
...
> **Why MINOR:** new user-visible capability (a per-page SEO title override + contentless-aware AI generation), additive; no public API removed or renamed. Precedence for description and og:image is unchanged in shape (explicit override, then AI-seeded, then theme fallback, then derived). The two new filter seams default to passthrough, so nothing changes until the theme registers listeners. Cookieless and cost model unchanged (auto-prepop still rides the existing daily-call ceiling). No em-dashes in emitted copy.

### New
- **Per-page SEO title override** (`_sn_seo_title`): a field in the "Signal & Noise" meta box, on any post or Page, that replaces the page-title segment of the `<title>` tag, `og:title`, and `twitter:title` at once (the site name is still appended). Empty falls back to the real page title. Registered `show_in_rest`, sanitized `sanitize_text_field`, saved on the existing nonce + `edit_post` cap-guarded path. Resolver: `sn_seo_resolve_singular_title()`.
- **Theme-fallback seams** `sn_seo_singular_title` and `sn_seo_singular_og_image` — plugin-side filter hooks that default to passthrough. The companion theme can register listeners to supply per-route title/image fallbacks (follow-up; not built here).

### Improved
- **AI meta-description and excerpt generation now work on contentless Pages.** New `snt_ai_post_signal()` returns `post_content` when present (byte-identical to the old path, zero regression for normal posts) and, when the body is empty, synthesizes a signal from the page title + the theme's `sn_seo_singular_description` fallback + slug. Both generators consume it, so their "Generate with AI" buttons produce copy on template Pages instead of returning a 422.
- **Publish-time auto-prepopulate reaches contentless Pages.** The word-count gate (`snt_prepop_passes_content_gate()`) now also admits a published Page whose body is empty but whose title yields a resolvable signal, while still rejecting thin, non-empty drafts. Already-published Pages are covered by the manual Generate button (prepop fires on entering publish, not retroactively).
- **OG card dek no longer blank on contentless Pages.** `sn_og_card_dek_source()` extends the card's dek/excerpt line to fall back to the curated `sn_seo_singular_description` after `post_excerpt` and content-derived text.
...
> **Why MINOR:** new user-visible surfaces (Movers, Pulse, uptime tiering, clamps) on an unchanged contract surface — no ability/route/settings removal or rename, no schema change, and the `snt_analytics_after_overview` seam keeps firing (test-asserted).

### New
- `inc/analytics-panels.php` — the panel primitive: the ONE place that emits panel chrome (postbox shell + crisp-console marker) and the row-clamp wrapper. All 13 formerly-inline postbox echoes in `analytics-admin-render.php` render through it; panels cannot drift apart again (`tests/analytics-tokens.php` pins the vocabulary).
- `inc/analytics-movers.php` — the Movers query (views delta vs `sn_analytics_prior_window`, dropped-out paths count as negative movers, zero-delta paths are noise) + the rail tile (5 rows, muted views beside signed deltas, Posts-view link).
- `inc/analytics-header-region.php` — the shared header frame (controls, class strip, Overview + rail grid, Pulse strip, uptime detail panel, the still-firing seam) rendered on every shared-chrome tab; returns the totals for the tail empty-hint.
- The Pulse strip: scroll/read-time band micro-bars with the dominant band named, bot-share window average with a bézier microspark (the shared smooth-path helper — no angular polylines anywhere), today-so-far from the already-fetched series. Cells self-omit when dataless; the strip renders nothing on a fresh install.
- Uptime lazy detail: `sn_uptime_status_rail_strip()` (status tier, no eager detail attribute) + `sn_uptime_status_detail_panel()` (collapsed; `assets/uptime-status.js` fetches `detail:true` on the first `sn-an-panel-open` event, once-guarded).
- **The four Dashboard-home widgets pair with the redesign** (owner call): Analytics — Overview gains a today-so-far line (the daily series' last bucket, zero new queries), a compact Movers section (the redesign's shared 15-min cache), and prior-period tooltips on every delta badge; Analytics — Top content gains proportional share bars behind both lists (CSS `--sn-aw-share`, zero new data); Login defense gains a 7-day blocked microspark riding the SAME cached headline (one extra AE query per 10 minutes, shared by every consumer); S&N Health names the open advisory queue in both the all-clear and findings sublines (`sn_health_advisory_total`) — information, not alarm. All guarded `function_exists`, all fixtures extended.

### Improvements
- Content view regrouped (`inc/analytics-view-content.php`); technology/geography/engagement/quality/events extracted to `inc/analytics-view-*.php` (the dispatcher is delegation-only, 590 → 477 lines); referrer categories retitled + compacted to chips with views/visits tooltips; the class strip names the automated share of all traffic; KPI numerals adopt the v8.0.3 glance contract (1.35rem/600/tabular); `uptime-status.css` drops the frame chrome the primitive now owns.
- Row clamps: Top pages 10 visible, dimension tables 5 (per-panel override arg), entry/exit/low-engagement 5 — full data stays in the DOM, "View all N" toggles, zero extra queries.
- Responsive: grids hold two columns to 782px; the stacked header rail goes side-by-side; journeys 3 → 2 → 1 across 960/782.

### Fixed
- The uptime monitor no longer fires the detail-tier Better Stack calls on every Analytics page load — the strip costs the status tier alone; detail is expand-only (HTTP-tripwire fixtures pin zero-remote-on-render and the once-guard).

## [8.4.5] - 2026-07-03: Apply no longer resurrects judged pairs — verdicts are ID-keyed

**Headline:** Owner report on v8.4.4: the link checks were "still doing the same." The v8.4.4 convergence fix was correct but only covered the no-apply path. Every verdict key embedded the posts' `post_modified_gmt` stamps, and Apply writes through `wp_update_post`, which bumps that stamp — so applying ONE good suggestion orphaned every other stored verdict involving that post. The skip verdicts just paid for stopped matching, those pairs re-nominated on the next scan, re-rendered, and re-billed on the next Suggest. Judging-then-applying is the intended workflow, and it partially reset its own convergence every time. Verdict rows are now **keyed on the post-id pair only**, with the stamps (and ids) stored inside the payload where lookups compare them: same invalidation semantics, but the row survives stamp changes and gets overwritten in place instead of orphaning per content generation. Apply then **restamps its own post's sibling rows** after the write — our splice is not an owner edit, so the AI's judgments of the post's other pairs still stand. A real editor save never restamps, so it re-nominates everything for that post exactly as before. Pre-v8.4.5 stamp-keyed rows are read through a fallback (findable exactly while their stamps still match, as always) and migrated forward on hit, so no pre-upgrade judgment is re-billed.

> **Why PATCH:** bugfix on v8.4.x behavior; no new capability, no API or settings-schema change. Verdict option prefixes are unchanged (`sn_link_verdict_` / `sn_pair_verdict_`), so the prune sweep covers both key generations.

### Fixed
- Verdict keys are ID-only via new `snt_ai_pair_verdict_key()` / `snt_ai_link_verdict_key()`; payloads carry `src_id` / `tgt_id` / `src_mod` (+ `tgt_mod` for pairs). New `snt_ai_verdict_lookup_pair()` / `snt_ai_verdict_lookup_link()` centralize the stamp comparison for both suggest caches and both scan suppressors (the key format previously lived in four places, which is how this class of bug hides).
- `snt_ai_link_apply_impl()` calls new `snt_ai_verdict_store_restamp()` after a successful write: every verdict row where the applied post appears as source or target gets that side's stamp updated to the post-apply value, `ts` preserved (restamping is not a fresh judgment). Legacy rows without ids are skipped.
- Regression fixtures across all four suites: judged siblings stay suppressed through an apply, a real edit still re-nominates, re-suggests overwrite the same row (no orphan pileup per stamp generation), and legacy rows suppress once then migrate. TDD red (17 failing assertions + 1 fatal) → green.

## [8.4.4] - 2026-07-02: One Suggest All pass reaches quiet — judged pairs keep their cap slots

**Headline:** Owner report after the v8.4.x verdict-store fixes: link opportunities still demanded manual re-run cycles — Suggest All → Re-run scan → Suggest All → … until the findings finally hit zero. The verdicts were durable and race-free by then; the treadmill was an ordering bug in the scans themselves. Both link checks ran judged-noise suppression BEFORE the per-source cap, so judging the rendered top-3 (pairs) or top-5 (mentions) freed those cap slots, and the very next scan promoted the next-ranked unjudged candidates into them — every cycle surfaced a fresh batch instead of converging. A judged pair now CONSUMES its cap slot while rendering nothing: the cap means "top-N scored candidates per source, judged or not," so one Suggest All pass plus one re-scan reaches a quiet, honest state, and the row copy ("No link to apply — clears on next scan") is finally literally true — the row clears and nothing takes its place. Editing either post still re-nominates naturally (verdict keys carry the modified stamps), and weaker never-rendered candidates stay out until content actually changes. Zero new AI spend: the fix is two loop reorders, not more judging.

> **Why PATCH:** scan-behavior bugfix (convergence); no new capability, no API or settings-schema change, verdict keys and shapes untouched.

### Fixed
- `sn_health_check_link_opportunities()`: the per-source cap check now runs before judged-noise suppression, and a judged pair increments the source's slot count without rendering — re-scans no longer promote next-ranked unjudged pairs into freed slots. Fixtures pin the full-judging (source goes quiet in one pass) and partial-judging (no backfill) behaviors against a deterministically ranked corpus.
- `sn_health_check_unlinked_mentions()`: same fix — a judged mention counts toward `SN_HEALTH_MENTIONS_MAX_PER_SOURCE` without rendering. Structural suppression (mentions that can never be spliced) stays non-consuming: it is deterministic noise that never enters the AI pipeline and must not crowd out actionable pairs. Fixtures mirror the pairs-check scenarios on the 6-eligible-targets corpus.
