...
- **Provenance** — cryptographic provenance for Notes: every publish/edit is Ed25519-signed, content-hashed, and Bitcoin-anchored via OpenTimestamps, then mirrored to a public [git ledger](https://github.com/juanlentino/signal-and-noise-provenance). Readers get a byline verification chip + expandable record panel (with clickable Bitcoin-block links) and can independently verify any Note — no trust in the site required. Backed by a dedicated [`sn-provenance` Cloudflare Worker](https://github.com/juanlentino/sn-provenance-worker); the plugin owns the canonical form, chain, and admin panel
